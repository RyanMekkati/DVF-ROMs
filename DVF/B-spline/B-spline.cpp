...
int main(int argc, char* argv[])
{
    ...
    for (auto &seriesFiles : allSeriesInputs) {
        ...
        for (unsigned int i = 0; i < 3; ++i) {
            physDim[i]  = fixed->GetSpacing()[i] *
                        static_cast<double>(fixed->GetLargestPossibleRegion()
                                                .GetSize()[i] - 1);
            meshSize[i] = 32;  // try 8,16,32 to tune spatial resolution
        }
        ...
        // Create and fill bound arrays of the right size:
        using OptimizerType    = itk::LBFGSBOptimizerv4;
        OptimizerType::BoundSelectionType boundSelect(numParameters);
        OptimizerType::BoundValueType     upperBound(numParameters);
        OptimizerType::BoundValueType     lowerBound(numParameters);

        boundSelect.Fill(0);     // 0 = “unbounded” for every parameter  
        upperBound.Fill(0.0);    // ignored since boundSelect is 0  
        lowerBound.Fill(0.0);    // ignored since boundSelect is 0  
        ...
        // 6) Transform → dense DVF
        DisplacementFilterType::Pointer dispFilter = DisplacementFilterType::New();
        ...
        // …VTK visualization (gated by --viz)…
        }
